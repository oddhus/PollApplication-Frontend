...
import { Typography, RadioGroup, Radio, FormControlLabel, Grid } from "@material-ui/core";
import { getDaysHoursMinFroSec, getSecondsFromDdHhMm } from "../utils/calculateTime"
...
const POLL_ATTRIBUTES = ['id', 'question', 'duration', 'startTime', 'visibilityType', 'owner']
...
class AdminPage extends Component {
  ...
  onPageChanged = page => {
    ...
    let lastElement = elements > this.state.data.length ? this.state.data.length : elements
  ...
  getList() {
    return this.state.data.slice(this.state.firstElement, this.state.lastElement)
  }
  ...
  onClickEdit(object) {
    if (this.state.tabValue === 0) {
      ...
      this.props.history.push({ pathname: `/polls/${objToEdit.id}`, state: objToEdit });
    }
  }
    ...
    return (
      <div>
        ...
        < Table
          ...
          data={this.getList()}
          attributes={this.state.tabValue === 0 ? USER_ATTRIBUTES : POLL_ATTRIBUTES}
